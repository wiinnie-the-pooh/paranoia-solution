...
:
    BlockLduSolver<Type>(fieldName, matrix),
...
)
{
    ...
    word segSolverName(solverData_);
    solverData_.putBack(segSolverName);
    ...
    )
    {
        ...
        // Rewind solver data for use with the next component and burn
        // the first token
        solverData_.rewind();
        word burnSegregated(solverData_);
